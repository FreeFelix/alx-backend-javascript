...
describe('API integration test', () => {
  ...
  /**
   * Test case for POST request to '/login' endpoint.
   * @name POST /login
   * @function
   * @memberof module:API integration test
   * @inner
  ...
  it('POST /login returns valid response', (done) => {
    // Sending a POST request to the API_URL + '/login' with JSON body
  ...
  /**
   * Test case for GET request to '/available_payments' endpoint.
  ...
  it('GET /available_payments returns valid response', (done) => {
    // Sending a GET request to the API_URL + '/available_payments'
    request.get(`${API_URL}/available_payments`, (_err, res, body) => {
      // Asserting that the status code of the response is 200 (OK)
      expect(res.statusCode).to.be.equal(200);
      // Asserting that the parsed body of the response matches the expected JSON object
